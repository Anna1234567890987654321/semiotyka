...
ageGenderModel <-  ulam(
  alist(
    netEstimate ~ dnorm( mu , sigma ) ,
    mu <- baseline + a[gender] * age_yrs + gb[gender] ,
    baseline ~ dnorm(400,100) ,
    a[gender] ~ dnorm(1,.5) ,
...
shonubi <- data.frame(age_yrs = 28, gender = 1, balloons = 103)
...
balloonsModel <-  ulam(
  alist(
    ...
    baseline ~ dnorm(200,100) ,
    b ~ dnorm(10,5) ,
    sigma ~ dunif( 0 , 100 )  # Increase the upper limit
  ) , data=balloonsData, log_lik = TRUE )
...
shonubiBalloonsPred <-  8 * sim(balloonsModel, shonubi)
...
shonubiBalloonsPred2 <-  sim(balloonsModel, shonubi2)
